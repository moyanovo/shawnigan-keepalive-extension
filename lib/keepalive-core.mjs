...
export const DEFAULT_SMART_INTERVAL_ENABLED = true
export const SMART_IDLE_THRESHOLD_MINUTES = 10
export const SMART_CLOSE_CHECK_MINUTES = 9
export const SMART_FRESH_CHECK_MINUTES = 3
export const MAX_DETAIL_ITEMS = 3
export const MAX_STORED_TAB_STATUSES = 20
export const AUTHENTICATION_URL_MARKERS = Object.freeze([
  'login',
  'signin',
  'sign-in',
  'blackbaudid',
...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  smartIntervalEnabled: DEFAULT_SMART_INTERVAL_ENABLED,
  lastSmartIntervalMinutes: DEFAULT_INTERVAL_MINUTES,
  isRunning: false,
  currentTrigger: null,
  lastRunAt: null,
  lastRunDurationMs: 0,
...
function normalizeTabStatus(entry) {
  ...
  const userStatus = normalizeUserStatusSnapshot(entry.userStatus)
...
export function normalizeUserStatusSnapshot(value) {
  if (!value || typeof value !== 'object') return null

  const tokenValid = value.TokenValid ?? value.tokenValid
  const minutesSinceActive = Number(value.MinutesSinceActive ?? value.minutesSinceActive)
  const authUsingBbid = value.AuthUsingBbid ?? value.authUsingBbid
  const currentVersion = value.CurrentVersion ?? value.currentVersion
  const snapshot = {}

  if (typeof tokenValid === 'boolean') {
    snapshot.tokenValid = tokenValid
  }

  if (Number.isFinite(minutesSinceActive) && minutesSinceActive >= 0) {
    snapshot.minutesSinceActive = minutesSinceActive
  }

  if (typeof authUsingBbid === 'boolean') {
    snapshot.authUsingBbid = authUsingBbid
  }
...
export function getSmartIntervalMinutes(pingResults, fallbackIntervalMinutes) {
  const fallback = normalizeInterval(fallbackIntervalMinutes)
  const statuses = Array.isArray(pingResults)
    ? pingResults
      .map((result) => normalizeUserStatusSnapshot(result?.userStatus))
      .filter(Boolean)
    : []

  if (!statuses.length) return fallback

  if (statuses.some((status) => status.tokenValid === false)) {
    return MIN_INTERVAL_MINUTES
  }

  const idleMinutes = statuses
    .map((status) => status.minutesSinceActive)
    .filter((value) => Number.isFinite(value) && value >= 0)

  if (!idleMinutes.length) return fallback

  const mostIdleMinutes = Math.max(...idleMinutes)

  if (mostIdleMinutes >= SMART_CLOSE_CHECK_MINUTES) {
    return MIN_INTERVAL_MINUTES
  }

  if (mostIdleMinutes < SMART_FRESH_CHECK_MINUTES) {
    return Math.min(SMART_FRESH_CHECK_MINUTES, fallback)
  }

  return Math.min(
    fallback,
    Math.max(MIN_INTERVAL_MINUTES, Math.ceil(SMART_IDLE_THRESHOLD_MINUTES - mostIdleMinutes)),
  )
}
...
export function normalizeSettings(storedSettings = {}) {
  ...
  return {
    ...
    smartIntervalEnabled: source.smartIntervalEnabled == null
      ? DEFAULT_SETTINGS.smartIntervalEnabled
      : Boolean(source.smartIntervalEnabled),
    lastSmartIntervalMinutes: normalizeInterval(source.lastSmartIntervalMinutes),
    isRunning,
...
export function isLikelyAuthenticationUrl(value) {
  ...
  try {
    ...
    normalizedValue = `${url.hostname}${url.pathname}${url.search}${url.hash}`.toLowerCase()
  } catch {
    normalizedValue = value.toLowerCase()
  }

  return AUTHENTICATION_URL_MARKERS.some((marker) => normalizedValue.includes(marker))
}
...
export function buildLastDetails(trigger, matchedCount, pingResults, options = {}) {
  ...
  if (!failureResults.length) {
    ...
    if (options.smartIntervalEnabled && Number.isFinite(options.smartIntervalMinutes)) {
      details.push(`Smart interval: next check in ${options.smartIntervalMinutes} min.`)
    }
...
export function buildLastTabStatuses(trigger, pingResults, checkedAt) {
  ...
  return pingResults
    .slice(0, MAX_STORED_TAB_STATUSES)
    .map((result) => {
      const userStatus = normalizeUserStatusSnapshot(result.userStatus)
...
export function annotateTabsWithLastStatus(tabs, lastTabStatuses) {
  ...
  return tabs.map((tab) => {
    ...
    return {
      ...
      userStatus: lastStatus?.userStatus || null,
    }
  })
}
